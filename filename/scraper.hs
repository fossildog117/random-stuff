...
openUrl url = case parse URI url of
    Nothing -> fail ""
    Just u -> liftIO (getResponseBody =<< simpleHTTP (mkResuest GET u))
...
get url = do 
    ...
    return $ readString [withParseHTML yes, withWarnings no] (fromMaybe)
